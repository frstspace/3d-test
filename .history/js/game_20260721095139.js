...
class Game {
    ...
    startGame() {
        ...
        if (!this.isMobile) {
            try {
                ...
                // Игнорируем, если Pointer Lock не поддерживается
            }
        }
    }
    ...
    _updatePlayer(delta) {
        // Ввод с джойстика
        const moveX = this.controls.moveX;
        const moveZ = this.controls.moveZ;

        // Поворот от джойстика прицела (значения сбрасываются после применения)
        const lookX = this.controls.lookX;
        const lookY = this.controls.lookY;
        this.controls.lookX = 0;
        this.controls.lookY = 0;

        // Обновляем углы обзора
        this.player.yaw -= lookX * 2.0;
        this.player.pitch -= lookY * 2.0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Стрельба по клику мыши (ПК)
    document.addEventListener('mousedown', (e) => {
        if (game && game.gameRunning && game.isPointerLocked && e.button === 0) {
            game.controls.shooting = true;
        }
    });
    document.addEventListener('mouseup', (e) => {
        if (game && game.gameRunning && e.button === 0) {
